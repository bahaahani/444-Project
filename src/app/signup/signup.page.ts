...
export class SignupPage implements OnInit {
 // private USR: Observable<users[]>;
  ...
  LoginForm:FormGroup;
  flag=true;
  constructor(public fb:FormBuilder,public d:AuthService,public t:ToastController){ 
    this.LoginForm=fb.group({
     ...
     Phone:['',Validators.compose([Validators.required,Validators.minLength(8),Validators.maxLength(30),
 ...
 async signup(){
  const mess=await this.t.create({
    ...
    if(this.LoginForm.valid&&this.LoginForm.controls['password'].value==this.LoginForm.controls['cpass'].value){
